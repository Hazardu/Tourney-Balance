...
mod:dofile("scripts/mods/TBPeregrinaje/changes/thp_stagger_changes")

-- Talent Changes
mod:dofile("scripts/mods/TBPeregrinaje/changes/talent_changes")

-- Weapon Changes
mod:dofile("scripts/mods/TBPeregrinaje/changes/weapon_changes")

-- Career Changes (Passives, Ultimates, etc.)
mod:dofile("scripts/mods/TBPeregrinaje/changes/career_changes")

--Enemies for Spicy
mod:dofile("scripts/mods/TBPeregrinaje/changes/SpicyEnemies")
...
function mod.tourney_balance_changes_apply(self)
	mod:talent_changes_apply()
	mod:weapon_changes_apply()
	mod:career_changes_apply()
...
mod.on_all_mods_loaded = function (self)
	...
	local originalFunc = pmod["load_peregrinaje"]
	pmod["load_peregrinaje"] = function (...)
		originalFunc(...)
			mod:tourney_balance_changes_apply()
			updateValues()
			NetworkConstants.server_controlled_buff_id.bits = 15
			NetworkConstants.server_controlled_buff_id.max = 32767
			mod:echo("Tourney balance compatible with pere enabled")
	end
end
...
-- 	return res
-- end
-- local diff_stagger_resist_cp = copy(BreedTweaks.diff_stagger_resist)
-- local stagger_duration_cp = copy(BreedTweaks.stagger_duration)
-- local stagger_duration_difficulty_mod_cp = copy(BreedTweaks.stagger_duration_difficulty_mod)



-- local CMD_HELP = [[
-- Adds weapon changes and selected skill/career changes from
-- Usage: /peretb 
-- ]]
-- mod:command("peretb", CMD_HELP, function ()
	
--     mod:echo("Tourney balance compatible with pere enabled via command")
-- end)
